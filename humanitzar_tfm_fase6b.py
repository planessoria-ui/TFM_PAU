...
SRC = "/root/.claude/uploads/446d5a3f-407d-5d51-8cea-50e8348ad898/f2840c7d-TFM_Pau_Planes_IRTA_v3_4.docx"
PATH = "/home/user/TFM_PAU/TFM_Pau_Planes_IRTA_v4.docx"
...
# ---------------------------------------------------------------------------
# Capçalera de la taula de sensors: Nre. -> Unitats
# ---------------------------------------------------------------------------
for t in doc.tables:
    caps = [c.text.strip() for c in t.rows[0].cells]
    if 'Nre.' in caps:
        i = caps.index('Nre.')
        for par in t.rows[0].cells[i].paragraphs:
            for r in par.runs:
                if r.text.strip() == 'Nre.':
                    r.text = 'Unitats'
        amples = [2.7, 4.2, 2.5, 1.3, 2.6, 2.7]
        for j, w in enumerate(amples):
            for row in t.rows:
                row.cells[j].width = Cm(w)
        print('capçalera Nre. -> Unitats')
